...
{
  int n; 
  printf("enter no of fractions you want to add");
  scanf("%d",&n);
  return n;
}
...
void output(int n, Fraction f[n], Fraction sum)
{
  ...
  printf("%d/%d=%d/%d=%d/%d",f[i].num,f[i].den,sum.num,sum.den,sum.num/simplification_factor,sum.den/simplification_factor);
}
